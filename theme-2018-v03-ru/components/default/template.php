...
if(strpos((string)'x' . HIDE_TEMPLATES, (string)$template_id) == false){
    ...
    //$mouseover = ($main_width != 0) ? "onmouseover=\"showtrail('" . $template->getMainPreview() . "','Шаблон " . $template->getId() . "'," . $main_width . "," . $main_height . ")\"" : "";
        ...
        <div class="template_info">
           <?php if($template->categories[0] == true){
            ...
            <div class="btn-box">
             ...
             <a href="<?php echo S_SITE_URL ?>demo/<?php echo $id ?>.html" target="_blank" class="btn3">Демо</a>
             <a href="<?php echo $template->getUrl()?>" class="btn">Подробнее</a>
			 <?php }else{ ?>
             <a href="<?php echo $template->getUrl()?>" class="btn" style="margin-top:20px">Подробнее</a><?php } ?>
            </div>
        </div>
    </div>
<?php } else { ?>
...
<?php } else if ( WHERE_WE_NOW == 'index' ) { ?>
    <div id="template_<?php echo $template->getId()?>" class="col-lg-3 col-md-4 col-sm-6 <?php if (FEATURED_SLIDER_OFF == 'true'){echo 'template';}else{echo 'swiper-slide';}?>">
        ...
        <div class="template_info">
            ...
            <?php else: ?>
                <div class="price_tag">
                    <?php echo util::template_price ($template->getRegularPrice()) ?><?php echo $template->getCurrencySymbol();?>
                </div>
            <?php endif; ?>
			<?php
...
<?php } else { ?>
    <div data-equal-group="5" id="template_<?php echo $template->getId()?>" class="col-lg-3 col-md-4 col-sm-6 template">
        ...
        <div class="template_info">
            <?php if (defined('CALCULATE_DISCOUNT') && CALCULATE_DISCOUNT == 1): ?>
                <div class="price_tag discount">
                    <span class="old-price"><?php echo util::template_price ($template->getRegularPrice()) ?><?php echo $template->getCurrencySymbol();?></span>
                    <span>
                        <?php echo util::template_price ($template->getRegularPrice() * ((DISCOUNT < 0.7) ? 0.7 : DISCOUNT )); ?><?php echo $template->getCurrencySymbol();?>
                    </span>
                </div>
            <?php else: ?>
                <div class="price_tag">
